...
#model, preprocess = clip_module.load('ViT-L/14', device)
...
def clip_predict(idx, dataset:MultipleDatasets, text_prompts:list, random_crops=False, num_crops=5, model_name='LesionCLIP'):
    ...
    if random_crops:
        text_inputs = torch.cat([clip_module.tokenize(c) for c in text_prompts]).to(device)
        ...
        image_input = preprocess(image).unsqueeze(0).to(device)
        text_inputs = torch.cat([clip_module.tokenize(f"{c}") for c in text_prompts]).to(device)
