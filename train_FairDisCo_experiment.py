...
log_file = f"Logs/fairdisco_dermie_experiment_{experiment_timestamp}.txt"
...
with open(log_file, 'w', encoding='utf-8') as f:
    f.write(f"FAIRDISCO DERMIE COMPARISON EXPERIMENT\n")
    f.write(f"Python Filename: {os.path.basename(__file__)}\n")
    f.write(f"Experiment Timestamp: {experiment_timestamp}\n")
    f.write(f"Experiment Design: Compare FairDisCo performance with and without Dermie dataset\n")
...
for i, combination in enumerate(combinations):
    ...
    ### MODEL LOADING ###
    print(f"Creating FairDisCo model for {experiment_name}...")
    ...
    ### MODEL TRAINING ###
    print(f"Training FairDisCo model for {experiment_name}...")
    
    model, fig = train_model(model, train_dataloader, val_dataloader, device, alpha=alpha, num_epochs=num_epochs)
    loss_path = save_plot_and_return_path(fig, f'{experiment_name}_losses')

    # Extract the classification model for testing
    model = nn.Sequential(
...
print(f"\n=== FAIRDISCO EXPERIMENT COMPLETED ===")
print(f"Results logged to: {log_file}")

with open(log_file, 'a', encoding='utf-8') as f:
    f.write("="*80 + "\n")
    f.write("FAIRDISCO EXPERIMENT SUMMARY\n")
    f.write("="*80 + "\n")
    f.write("Two FairDisCo experiments completed:\n")
